...
from django.utils import timezone # <-- THIS WAS THE MISSING LINE
...
class FlightListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if date:
            ...
            try:
                # Use timezone.now().date() if date is not provided, but here date is provided
...
class SeatBookingView(APIView):
    ...
    def post(self, request, flight_id):
        ...
        seats_booked_count = len(seats)
        if seats_booked_count > flight.available_seats:
            return Response({"error": "Not enough seats available"}, status=status.HTTP_400_BAD_REQUEST)

        # --- START RE-CALCULATED PRICING LOGIC ---
        
        # 1. Get base dynamic price from flight
        # This is now the *only* price calculation.
        price_per_ticket = float(flight.dynamic_price(booking_date=timezone.now()))

        # --- FIX: All extra surge/discount logic is REMOVED ---

        # 4. Calculate final price
        final_price_per_ticket = price_per_ticket
        final_total_price = final_price_per_ticket * seats_booked_count
        # --- END RE-CALCULATED PRICING LOGIC ---


        # Create booking with the "locked-in" final price
        booking = Booking.objects.create(
            user=user,
            flight=flight,
            seats_booked=seats_booked_count,
            price_per_ticket=round(final_price_per_ticket, 2),
            total_price=round(final_total_price, 2)
            # PNR is generated by the model's save() method
        )
        ...
        flight.available_seats = max(0, flight.available_seats - seats_booked_count)
        flight.save()
...
class BookingCreateView(APIView):
    ...
    def post(self, request, flight_id):
        ...
        if seat_numbers:
            ...
            # --- START RE-CALCULATED PRICING LOGIC (for generic booking) ---
            # This is now the *only* price calculation.
            price_per_ticket = float(flight.dynamic_price(booking_date=timezone.now()))
            ...
            final_price_per_ticket = price_per_ticket
            final_total_price = final_price_per_ticket * seats_book
            # --- END RE-CALCULATED PRICING LOGIC ---
...
# This is the new (admin) view for all bookings
...
class PaymentCreateView(APIView):
    def post(self, request, booking_id):
        ...
        txn = f"TXN{int(timezone.now().timestamp())}"
...
class RazorpayOrderView(APIView):
    def post(self, request, booking_id):
        ...
        amount_in_paise = int(booking.total_price * 100)  # Razorpay expects paise
        currency = "INR"
